...
def main():
    ...
    while iter_idx <= config.train_iter:
        for train_batch in data_loaders['train']:
            # Parse data
            ...
            if (iter_idx % config.run_validation_every == 0 or
                ...
                val_stats = evaluation(eval_model, data_loaders['valid'], writer, config,
    ...
    final_val_stats = evaluation(eval_model, data_loaders['valid'], None, config,
                                 iter_idx, N_eval=config.N_eval,
                                 N_seg_metrics=300)
    fprint(f"FINAL VALIDATION STATS | {final_val_stats}", True)

    # Test evaluation
    name_model = config.run_name.split('_')[1]
    name_outputs = ['recon', 'apc', 'mask']
    if name_model == 'genesis':
        name_outputs.append('shp')
    model.eval()
    for phase in ['test', 'general']:
        ...
        if name_model == 'genesis':
            ...
            if model.K_steps > model.std.shape[-1]:
                size_pad = model.K_steps - model.std.shape[-1]
        ...
        for data in data_loaders[phase]:
            ...
            for _ in range(config.num_tests):
                with torch.no_grad():
                    _, _, stats, att_stats, _ = model(images)
                recon = stats.recon
        ...
        with h5py.File(os.path.join(logdir, '{}.h5'.format(phase)), 'w') as f:
